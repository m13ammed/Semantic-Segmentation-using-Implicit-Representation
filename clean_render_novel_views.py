...
@gin.configurable()
def export_images(labels, target_images, depth_data, show_only=False, scene_name="1", frame_ids = [], output_segmentation = './out/', colored =False, threshold=3000):
    ...
    for frame_id, label, imgg, depth in zip(frame_ids, labels, target_images, depth_data):
        ...
        if(np.sum(seg==int(cls_id))<threshold): continue
