...
import torch
import numpy as np
# A class that performs the training of the model given a training dataset data loader, the optimizer,
# and the loss criterion.
class Train:
	...
	def run_epoch(self, iteration_loss=0, epoch_num=0):
		...
		for step, batch_data in enumerate(self.data_loader):
			...
			if self.scaler is None: 
       			#initilaize the the gradscaler for amp training
	...
	def preppare_input(self, batch):
		...
		if not self.data_loader.dataset.allow_gen_lables:
			...
			labels_2d[idx_to_ren] = labelss #replace placeholder with the generated label only where there is no label pre generated
