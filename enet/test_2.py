...
class Test2:
	...
	def run_epoch(self, iteration_loss=False, epoch_num = 0):
		...
		for step, batch_data in enumerate(self.data_loader):
			...
			#self.writer.add_scalar('loss_step/test', losss_item, (epoch_num)*len(self.data_loader) + step ) 
			#self.writer.add_scalar('time_step/test', ((endTime - startTime)*1000), (epoch_num)*len(self.data_loader) + step)
		...
		#self.writer.add_scalar('loss_epoch/test',  avg_loss, (epoch_num+1))
		#self.writer.add_scalar('time_epoch/test', ((endTime - first_time)*1000), (epoch_num+1))
		#self.writer.add_scalar('miou_epoch/test', miou, (epoch_num+1))
		if self.first:
			self.first = False
		
		#for key, iou_ in zip(self.class_encoding, np.nan_to_num(iou,nan=-1)):
		#	self.writer.add_scalar('test_iou/'+key, float(iou_), (epoch_num+1))
