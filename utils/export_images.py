...
import gin
@gin.configurable()
def export_images(labels, target_images, depth_data, show_only=False, scene_name="1", frame_ids = [], output_segmentation = './out/', colored =False, save_depth=False):
    ...
    for frame_id, label, imgg, depth in zip(frame_ids, labels, target_images, depth_data):
        ...
        if(colored):
            ...
            if np.ndim(tensor)>3:
                ...
                tensor = tensor[0]
            img_rgb = Image.fromarray(tensor)
        ...
        if(show_only):
            ...
            if(colored):
                img_rgb.save(os.path.join(path_scene,str(frame_id)+".png"))
            np.save(os.path.join(path_scene,str(frame_id)+".npy"),img)
            if(save_depth): np.save(os.path.join(path_scene,str(frame_id)+"_depth.npy"),depth_out)
